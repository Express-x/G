...
raw_dataset = datasets.load_dataset("json", data_files={"train": "huggingface_models/train.json", "test": "huggingface_models/eval.json"})
...
# Training
###########
# Define the layers to fine-tune
layers_to_fine_tune = [
    # First 5 layers
    "model.layers.0",
    "model.layers.1",
    ...
    "model.layers.3",
    "model.layers.4",
    # Middle 5 layers (assuming a total of 32 layers)
    "model.layers.14",
    "model.layers.15",
    "model.layers.16",
    "model.layers.17",
...
# Update the LoRA config to target specific layers
peft_config["target_modules"] = layers_to_fine_tune
